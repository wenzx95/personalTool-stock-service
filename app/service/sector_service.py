...
class SectorService:
    ...
    def get_sector_list(self) -> List[Dict]:
        """获取板块列表（内部方法，供get_sector_review_table使用）"""
        logger.info("[SectorService] 开始获取板块列表")
        start_time = time.time()
        try:
            logger.info("[SectorService] 调用crawler.get_sector_list()")
    ...
    def get_sector_review_table(self) -> List[Dict]:
        ...
        try:
            ...
            logger.info("[SectorService] 步骤2: 格式化表格数据（仅核心字段）")
            logger.info(f"[SectorService] 调试: sectors长度={len(sectors)}, sectors类型={type(sectors)}")
            ...
            for i, sector in enumerate(sectors, 1):
                ...
                # 调试：检查sector数据结构
                if i == 1:
                    logger.info(f"[SectorService] 调试: 第一个sector数据示例: {sector}")
                ...
                row_data = {
                    ...
                    # 计算上涨比例
                    'up_ratio': round((up_count / total_stocks) * 100, 2) if total_stocks > 0 else 0.0,
                    # 计算涨停占比
                    'limit_up_ratio': round((limit_up_count / total_stocks) * 100, 2) if total_stocks > 0 else 0.0,
                }
                
                # 调试：检查第一条数据
                if i == 1:
                    logger.info(f"[SectorService] 调试: 第一条table_data: {row_data}")
                
                table_data.append(row_data)
            
            logger.info(f"[SectorService] 步骤2完成: 已构建 {len(table_data)} 条table_data")
            ...
            logger.info(f"[SectorService] 调试: 返回table_data类型={type(table_data)}, 长度={len(table_data)}")
            if table_data:
                logger.info(f"[SectorService] 调试: 返回的第一条数据: {table_data[0]}")
            return table_data
        except Exception as e:
            elapsed = time.time() - start_time
